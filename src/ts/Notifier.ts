class Notifier {
	...
	// イベントタイプをキーにした連想配列です。
	// 各値には登録されているイベントリスナーの配列が格納されています。
	private _typeList:{[index:string]:Function[]};
	 ...
	 */
	public on(type:string, listener:Function):Notifier {
		var typeList:{[key:string]:Function[]}, listenerList:Function[];

		typeList = this._typeList = this._typeList || {};
		listenerList = typeList[type];
		if (listenerList) this.off(type, listener);
		listenerList = typeList[type];
	 ...
	 * @return {Notifier}				メソッドチェーン用の自身のオブジェクト。
	 */
	public off(type:string, listener:Function):Notifier;
	public off(type:string):Notifier;
	public off():Notifier;
	 ...
	 */
	public off(type?:string, listener?:Function):Notifier {
		var i:number, len:number, typeList:{[key:string]:Function[]}, listenerList:Function[];

		// all listeners is removed.
		if (!type && !listener) {
			delete this["_typeList"];

			// all listeners of the type is removed.
		} else if (!listener) {
			typeList = this._typeList;
			if (!typeList) return this;

			delete typeList[type];
			if (Object.keys(typeList).length == 0) delete this["_typeList"];
	 ...
	 */
	public check(type:string, listener:Function):boolean;
	...
	public check(type?:string, listener?:Function):boolean {
		...
		if (!type && !listener) {
			...
			len = listenerList.length;
			for (i = 0; i < len; i++) {
				if (listenerList[i] == listener) return true;
			}
			return false;
		}
	}
	 ...
	 * @return {Notifier}			メソッドチェーン用の自身のオブジェクト。
	 */
	public notify(type:string, params:any[]):Notifier;
	public notify(type:string):Notifier;
